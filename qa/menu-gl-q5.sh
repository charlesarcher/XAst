#!/usr/bin/env bash
# qa/menu-gl-q5.sh — Q5 GL leg driver + assertions (task 44a).
#
# Runs test/harness/scripts/menu-gl.script through the harness in frame
...
#
# Usage: qa/menu-gl-q5.sh [--out DIR] (default /tmp/opencode/q5-run)
# Exit 0 = PASS; 1 = FAIL.
...
# ---- preflight -------------------------------------------------------------
ldd ./XAsteroids | grep -q glfw || { note "FAIL: ./XAsteroids is not the GL flavor"; exit 1; }
...
XAST_QA_MENU_RECTS=1 ./obj/harness --seed 12345 --script test/harness/scripts/menu-gl.script \
...
note ""
if [ $fail -eq 0 ]; then note "RESULT: PASS (Q5 GL leg)"; exit 0
else note "RESULT: FAIL (Q5 GL leg)"; exit 1; fi
